...
言葉の経験学習・初回比較版は、開始状態・学習四判定・保持方針・六種類の対応表現と検証基準を合意済み。次チャットで既存版と分離した実装へ進む予定。新方式はまだ未実装で、調整値や一部の保存・資源運用の詳細は残る。再開点は[正本](core/experimental_word_learning.md)の「実装開始前の到達点と残る調整」を参照。

言葉の経験学習・比較実験版では、お願い・説明を受け取る土台を持って始めるか、その土台から育てるかも開始時に選べる方針を追加。詳細・未決定事項は[正本](core/experimental_word_learning.md)を参照する。

入力から文章化までの六段階と、知識・育った関連／今考えている文脈／残した記録の三分類は合意済み。同正本の構想欄に具体化資料を記録し、保持期間・更新条件・選択規則等の詳細は実装開始前の検討段階を維持する。
...
| 言葉の経験学習・比較実験版 | 未実装・構想段階 | 既存版を維持して比較。初回比較は食事・睡眠・散策・会話で、声かけ六種類（現実の出来事、否定・言い直し等）を対象とする。Steam公開体験版は基本職6種への興味・弟子入りと少しの仕事経験まで含める。Steamで新旧両方を公開する想定ではない。詳細は正本で壁打ち中 |
